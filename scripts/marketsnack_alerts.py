...
from data_sources.sweeps import (  # noqa: E402
    by_buy_volume, calls_only, drop_0dte, drop_multileg, filter_contracts,
...
def format_alert(subject: str, body: str, when: str | None = None) -> str:
    ...
    parsed = drop_0dte(calls_only(drop_multileg(parse_contracts(body, fallback_time=when))))
    contracts = by_buy_volume(filter_contracts(parsed))[:3]
    if not contracts:
